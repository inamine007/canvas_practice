...
function initialize(){
  ...
  // backgroundWave = new BackgroundWave(util, 200);
  ...
  // 自機キャラクターを初期化する
  viper = new Viper(ctx, 0, 0, 100, 100, './image/game2.png');
  ...
  viper.setVectorFromAngle(degreesToRadians(270));
  ...
  // ボスキャラクターを初期化する
  boss = new Boss(ctx, 0, 0, 128, 128, './image/game2_boss.png');
  ...
  boss.setVectorFromAngle(degreesToRadians(270));

  // 敵キャラクター（小）を初期化する
  for(i = 0; i < ENEMY_SMALL_MAX_COUNT; ++i){
    enemyArray[i] = new Enemy(ctx, 0, 0, 48, 48, './image/game2_enemy.png');
    // 敵キャラクターはすべて同じショットを共有するのでここで与えておく
  ...
  for(i = 0; i < ENEMY_LARGE_MAX_COUNT; ++i){
    enemyArray[ENEMY_SMALL_MAX_COUNT + i] = new Enemy(ctx, 0, 0, 64, 64, './image/game2_enemy.png');
    // 敵キャラクターはすべて同じショットを共有するのでここで与えておく
...
function sceneSetting(){
      ...
      for(let i = 0; i < ENEMY_SMALL_MAX_COUNT; ++i){
        if(enemyArray[i].life <= 0){
          ...
          if(scene.frame % 60 === 0){
            ...
            // 進行方向は 30 度の方向
            e.setVectorFromAngle(degreesToRadians(160));
          }else{
            // 右側面から出てくる
            e.set(CANVAS_WIDTH, CANVAS_HEIGHT - e.height, 2, 'default');
            // 進行方向は 150 度の方向
            e.setVectorFromAngle(degreesToRadians(200));
          }
          break;
        }
      }
  ...
  scene.add('invade_boss', (time) => {
    ...
    if(scene.frame === 0){
      ...
      boss.set(CANVAS_WIDTH, CANVAS_HEIGHT / 2, 250);
...
function render(){
  ...
  util.drawRect(0, 0, canvas.width, canvas.height, '#3E83C8');
  t++;
  ...
  // util.drawWave('#43c0e4', 1, 3, 0, t / 30);
