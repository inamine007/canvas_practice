  ...
  update(){
    ...
    switch(this.type){
      ...
      case 'wave':
        ...
        this.position.x -= 2;
        this.position.y += Math.sin(this.frame / 10);
  ...
  update(){
    ...
    switch(this.mode){
      ...
      case 'invade':
        ...
        if(this.position.x < CANVAS_WIDTH - 200){
          this.position.x = CANVAS_WIDTH - 200;
          this.mode = 'floating';
          this.frame = 0;
        }
        break;
...
class BackgroundWave {
   ...
   * @constructor
   * @param {CanvasRenderingContext2D} ctx - 描画などに利用する 2D コンテキスト
   * @param {number} height - 波の高さ
   * @param {number} speed - 波の速度
   * @param {string} [color='#43c0e4'] - 波の色
   */
   constructor(util, height, color = '#43c0e4'){
    /**
     * @type {CanvasRenderingContext2D}
     */
    this.util = util;
  ...
  set(color){
    ...
    //this.util.drawWave(color[0], 1, 3, 0);//drawWave(canvas, color[0],0.5, 3, 0);とすると透過50%の波が出来る
  }
  ...
  update(){
    console.log('hoge')
    this.util.drawWave(this.color, 1, 3, 0);//drawWave(canvas, color[0],0.5, 3, 0);とすると透過50%の波が出来る
  }
}
