class Canvas2DUtility {
  ...
  */
  drawWave(color, alpha, zoom, delay, t) {
    this.context2d.fillStyle = color;//塗りの色
    this.context2d.globalAlpha = alpha;
    this.context2d.beginPath(); //パスの開始
    this.drawSine(t, zoom, delay);
    this.context2d.lineTo(this.canvasElement.width + 10, this.canvasElement.height); //パスをCanvasの右下へ
    this.context2d.lineTo(0, this.canvasElement.height); //パスをCanvasの左下へ
    this.context2d.closePath() //パスを閉じる
    this.context2d.fill(); //波を塗りつぶす
  }
  ...
  */
  drawSine(t, zoom, delay) {
    let unit = 100;
    let xAxis = Math.floor(this.canvasElement.height - 100);
    let yAxis = 0;
    // Set the initial x and y, starting at 0,0 and translating to the origin on
    // the canvas.
    let x = t; //時間を横の位置とする
    let y = Math.sin(x)/zoom;
    this.context2d.moveTo(yAxis, unit*y+xAxis); //スタート位置にパスを置く

    // Loop to draw segments (横幅の分、波を描画)
    for (let i = yAxis; i <= this.canvasElement.width + 10; i += 10) {
      x = t+(-yAxis+i)/unit/zoom;
      y = Math.sin(x - delay)/3;
      this.context2d.lineTo(i, unit*y+xAxis);
    }
  }
}